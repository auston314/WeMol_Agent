"""
Student SQL MCP Server (SSE Version) - Execute SQL queries on student database

This MCP server provides SQL query execution capabilities for the student database
via SSE (Server-Sent Events) transport.

Features:
- Execute SQL SELECT queries on student database
...
logger = logging.getLogger("student_sql_sse_server")
...
mcp = FastMCP("student_sql_server")


@mcp.tool()
def execute_sql_query(query: str) -> str:
    """Execute a SQL SELECT query on the student database and return results.
    ...
    Available tables:
    - students: Main student records table with columns: id, first_name, last_name, 
      gender, date_of_birth, class, email, phone, major, gpa, test_i, test_ii, 
      test_iii, test_iv, middle_term_exam, final_exam, final_score
...
def get_database_schema() -> str:
    """Get the complete schema information for the student database.
    ...
    try:
        ...
        if indexes:
            ...
            schema_text += "Indexes:\n"
            for idx in indexes:
                schema_text += f"  • {idx['name']}: {'UNIQUE' if idx['unique'] else 'INDEX'}\n"
...
def get_sql_examples() -> str:
    """Get a collection of useful SQL query examples for the student database.
    ...
    Returns:
        Collection of example SQL queries
    """
...
def get_database_stats() -> str:
    """Get comprehensive statistics about the student database.
...
@mcp.tool()
def get_table_preview(limit: int = 5) -> str:
    """Get a preview of the student table with sample records.
...
if __name__ == "__main__":
    logger.info("Starting Student SQL MCP Server (SSE) on port 3001...")
    logger.info("Database initialized with student records")
    logger.info("Available tools: execute_sql_query, get_database_schema, get_sql_examples, get_database_stats, get_table_preview")
